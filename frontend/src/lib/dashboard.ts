...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  const receivables = invoices.filter((invoice) => invoice.backendStatus === "pending" || invoice.backendStatus === "in_auction");
  const eligible = invoices.filter((invoice) => invoice.backendStatus === "pending");
...
function buildOpportunities(invoices: InvoiceListItem[], activeByInvoice: { invoice: InvoiceListItem; offers: DashboardOffer[] }[]): DashboardOpportunity[] {
  ...
  const eligible = invoices.find((invoice) => invoice.backendStatus === "pending" && invoice.offersCount === 0);
