...
export function MarketplacePageContent({ data }: { data: MarketplaceData }) {
  ...
  return <div className="mx-auto max-w-[1380px]"><header className="flex flex-wrap items-start justify-between gap-4"><div className="flex items-start gap-3"><PageBackLink href="/financier" label="Volver al inicio financiero"/><div><h1 className="text-[38px] font-bold leading-none tracking-[-.04em] text-navy">Marketplace</h1><p className="mt-1.5 text-[17px] text-[#1d54b7]">Encuentra facturas de empresas confiables y haz crecer tu capital.</p></div></div><article className="flex w-full items-center gap-4 rounded-xl bg-[#f0f4ff] px-5 py-3 sm:w-auto sm:min-w-[365px]"><span className="flex h-12 w-12 items-center justify-center rounded-xl bg-white text-[#175ee3]"><Icon name="wallet" size={25}/></span><div><p className="text-sm text-[#38517d]">Disponible para financiar</p><strong className="mt-1 block text-[25px] leading-none text-navy">{formatCents(data.availableCapitalCents)}</strong><p className="mt-1.5 text-xs text-[#1d54b7]">Fondos disponibles para nuevas oportunidades</p></div></article></header>
    <section className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><MarketplaceKpiCard label="Facturas disponibles" value={String(data.opportunities.length)} detail="Buscando financiamiento" icon="invoice" tone="blue"/><MarketplaceKpiCard label="Monto buscando financiamiento" value={`${formatCents(totalAmount)} MXN`} detail={`En ${data.opportunities.length} facturas`} icon="coins" tone="orange"/><MarketplaceKpiCard label="Retorno promedio" value={data.averageReturnRate === null ? "—" : `${data.averageReturnRate}%`} detail="De las oportunidades disponibles" icon="percent" tone="purple"/><MarketplaceKpiCard label="Deudores que ya conoces" value={data.knownDebtorsCount === null ? "—" : String(data.knownDebtorsCount)} detail="Oportunidades con deudores que has financiado antes" icon="shield" tone="green"/></section>
