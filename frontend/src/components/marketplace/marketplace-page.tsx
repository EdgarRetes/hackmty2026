...
export function MarketplacePageContent({ data }: { data: MarketplaceData }) {
  ...
  const filtered = useMemo(() => data.opportunities.filter((item) => matchesOpportunity(item, query, quickFilter, amountFilter, termFilter)).sort((a, b) => sortOpportunities(a, b, sort)), [data.opportunities, query, quickFilter, amountFilter, termFilter, sort]);
...
function matchesOpportunity(item: MarketplaceOpportunity, query: string, quick: QuickFilter, amount: AmountFilter, term: TermFilter): boolean {
  ...
  return matchesQuery && matchesQuick && matchesAmount && matchesTerm;
}
