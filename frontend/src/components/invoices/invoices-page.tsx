...
export function InvoicesPageContent({ invoices }: { invoices: InvoiceListItem[] }) {
  ...
  return <div className="mx-auto max-w-[1380px]"><h1 className="text-[38px] font-bold leading-none tracking-[-.04em] text-navy">Facturas</h1><p className="mt-1.5 text-[17px] text-[#52688f]">Consulta y administra las facturas de tu empresa.</p>
    ...
    {selected.length > 0 && <section className="mt-4 flex flex-wrap items-center gap-3 rounded-xl border border-[#d1baff] bg-[#f5efff] px-4 py-3"><span className="text-sm font-semibold text-navy">{selected.length} factura{selected.length === 1 ? "" : "s"} seleccionada{selected.length === 1 ? "" : "s"}</span><span className="text-xs text-[#52688f]">{selected.length < 2 ? "Selecciona al menos 2 para publicarlas como un solo paquete." : "Se publicarán juntas: un financiador puede pagar las 3 y quedarse con el rendimiento de todas."}</span><button onClick={handlePublish} disabled={selected.length < 2 || publishing} className="ml-auto flex h-10 items-center gap-2 rounded-lg bg-[#6330e8] px-5 text-sm font-semibold text-white transition hover:bg-[#5827c9] disabled:cursor-not-allowed disabled:opacity-50">{publishing ? "Publicando..." : "Publicar paquete"}</button><button onClick={() => setSelected([])} className="text-xs font-medium text-[#52688f] hover:text-navy">Limpiar selección</button></section>}
