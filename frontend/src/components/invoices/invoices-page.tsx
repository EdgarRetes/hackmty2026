...
export function InvoicesPageContent({ invoices }: { invoices: InvoiceListItem[] }) {
  ...
  return <div className="mx-auto max-w-[1380px]"><header className="flex items-start justify-between gap-4"><div className="flex items-start gap-3"><Link href="/" aria-label="Volver al inicio" className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-navy transition hover:bg-slate-100"><Icon name="chevronLeft" size={23}/></Link><div><h1 className="text-[38px] font-bold leading-none tracking-[-.04em] text-navy">Facturas</h1><p className="mt-1.5 text-[17px] text-[#52688f]">Consulta y administra las facturas de tu empresa.</p></div></div><button onClick={() => router.push("/invoices/package")} className="shrink-0 rounded-xl bg-navy px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#17315f]">Crear paquete</button></header>
