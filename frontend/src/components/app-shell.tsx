...
const primary: { label: string; icon: IconName; href: string }[] = [
  ...
  { label: "Ofertas", icon: "grid", href: "/" }, { label: "Financiamientos", icon: "cube", href: "/publications" },
...
function NavList({ items, activeSection }: { items: typeof primary; activeSection?: string }) {
  return <nav className="space-y-2.5">{items.map((item) => <a key={item.label} href={item.href} className={`flex h-12 items-center gap-5 rounded-xl px-4 text-[15px] font-medium transition ${item.label === activeSection ? "bg-gradient-to-r from-[#efffc3] to-[#f6ffd9] text-navy" : "text-navy hover:bg-slate-50"}`}><Icon name={item.icon} size={22}/><span>{item.label}</span></a>)}</nav>;
}

export function AppSidebar({ activeSection = "Ofertas" }: { activeSection?: string }) {
  return <aside className="fixed inset-y-0 left-0 z-20 hidden w-[226px] border-r border-slate-200 bg-white px-4 py-6 lg:flex lg:flex-col">
    ...
    <div className="mt-14"><NavList items={primary} activeSection={activeSection}/></div>
    <div className="mt-10 border-t border-slate-200 pt-7"><NavList items={secondary} activeSection={activeSection}/></div>
...
export function AppShell({ children, activeSection = "Ofertas" }: { children: React.ReactNode; activeSection?: string }) {
  return <div className="min-h-screen bg-[#fbfcfe]"><AppSidebar activeSection={activeSection}/><TopBar/><main className="px-5 py-6 md:px-7 lg:ml-[226px]">{children}</main></div>;
}
