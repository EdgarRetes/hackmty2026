...
export function OfferCard({ offer, onAccept, accepting }: { offer: Offer; onAccept?: (offerId: string) => void; accepting?: boolean }) {
  ...
  return <article className={`flex min-w-0 flex-col rounded-xl border bg-white p-3.5 shadow-[0_8px_24px_rgba(15,31,68,.025)] transition hover:-translate-y-1 hover:shadow-lg ${tone.border}`}>
    ...
    <button onClick={() => (offer.action === "Aceptar oferta" || offer.action === "Hacer oferta") && onAccept?.(offer.id)} disabled={accepting} className={`mt-4 h-10 rounded-lg text-sm font-semibold transition active:scale-[.98] disabled:cursor-wait disabled:opacity-60 ${tone.button} ${offer.tone === "lime" ? "text-navy" : tone.text}`}>{accepting ? "Enviando..." : offer.action}</button>
