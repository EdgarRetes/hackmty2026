...
export function OffersPageContent({ data }: { data: OffersPageData }) {
  ...
  return <div className="mx-auto max-w-[1280px]">
    ...
    {sorted.length > 0 ? <section className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{sorted.map((offer) => <OfferCard key={offer.id} offer={offer} onAccept={handleAccept} accepting={acceptingId === offer.id}/>)}</section> : <OffersEmptyState/>}
